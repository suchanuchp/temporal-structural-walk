...
from stellargraph.core.validation import require_integer_in_range, comma_sep
...
class TemporalStructuralRandomWalk(GraphWalk):
    """
    ...
    def __init__(
        self,
        graph,
        structural_graph: StellarGraph = None,
        cw_size=None,
        max_walk_length=80,
        ...
        self.structural_nbrs = self.init_structural_nbrs(structural_graph) if structural_graph is not None else None

    @classmethod
    def init_structural_nbrs(cls, structural_graph):
        # TODO: remove this and directly use neighbor_arrays
        results = {}

        # Iterate through each node in the structural graph
        for node in structural_graph.nodes():
            structural_neighbors, weights = structural_graph.neighbor_arrays(node, include_edge_weight=True)
            neighbors_array = structural_neighbors
            weights_array = weights

            results[node] = (neighbors_array, weights_array)
    ...
    def run(
        ...
        seed=None,
        include_same_timestep_neighbors=False,
        alpha=0.,
    ):
        """
        ...
        while num_cw_curr < num_cw:
            ...
            walk = self._walk(
                ...
                include_same_timestep_neighbors=include_same_timestep_neighbors, alpha=alpha,
            )
            if len(walk) >= cw_size:
                walks.append(walk)
    ...
    def _exp_biases(self, times, t_0, decay):
        if decay:
            diff = t_0 - np.array(times)
            return softmax(diff)

        return softmax(np.array(times) - t_0)

    def _linear_biases(self, sorted_weights):
        assert np.all(sorted_weights[:-1] <= sorted_weights[1:]), "weights should be pre-sorted"
        changes = np.concatenate(([True], sorted_weights[1:] != sorted_weights[:-1]))
        rank_array = np.cumsum(changes)
        return rank_array/np.sum(rank_array)
    ...
    def _step(self, node, time, bias_type, np_rs, include_same_timestep_neighbors, alpha=0.):
        """
        Perform 1 temporal step from a node. Returns None if a dead-end is reached.

        """
        if (self.structural_graph is None) or (np.random.rand() > alpha):
            neighbors, times = self.graph.neighbor_arrays(node, include_edge_weight=True)
            mask = (times >= time) if include_same_timestep_neighbors else (times > time)
            neighbors = neighbors[mask]
            times = times[mask]
            if len(neighbors) > 0:
                biases = self._temporal_biases(times, time, bias_type, is_forward=True)
                chosen_neighbour_index = self._sample(len(neighbors), biases, np_rs)
                assert chosen_neighbour_index is not None, "biases should never be all zero"
                next_node = neighbors[chosen_neighbour_index]
                next_time = times[chosen_neighbour_index]
                return next_node, next_time
            else:
                return None
        else:
            structural_temporal_nodes, st_biases = self.structural_nbrs[node]
            chosen_neighbour_index = self._sample(len(structural_temporal_nodes), st_biases, np_rs)
            next_node = structural_temporal_nodes[chosen_neighbour_index]
            next_time = time  # stay in the same timestep
            return next_node, next_time

    def _walk(self, src, dst, t, length, bias_type, np_rs, include_same_timestep_neighbors, alpha=0.):
        walk = [src, dst]
        node, time = dst, t
        tts = [t]
